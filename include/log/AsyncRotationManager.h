...
struct AsyncRotationRequest {
    ...
    AsyncRotationRequest() : requestTime(std::chrono::system_clock::now()), priority(0), manualTrigger(false) {}

    AsyncRotationRequest(const std::wstring& id, const std::wstring& fileName)
        : requestId(id), currentFileName(fileName), requestTime(std::chrono::system_clock::now()),manualTrigger(false),priority(0){}
};

/**
 * @brief 轮转请求比较器（优先级队列用）
 * @struct RotationRequestComparator
 * @attention 优先级高的请求排在前面，同优先级按请求时间排序
 */
struct RotationRequestComparator {
...
struct RotationTaskInfo {
    std::wstring                           taskId;        /*!< Task ID */
    std::wstring                           fileName;      /*!< File Name */
    RotationTaskStatus                     status;        /*!< Rotation Status */
    std::chrono::system_clock::time_point  startTime;     /*!< Start Time */
    std::chrono::system_clock::time_point  endTime;       /*!< End Time */
    std::wstring                           errorMessage;  /*!< Error message */
    int                                    priority;      /*!< Priority */

    RotationTaskInfo() : startTime(std::chrono::system_clock::now()),priority(0),status(RotationTaskStatus::Pending) {}

    RotationTaskInfo(const std::wstring& id, const std::wstring& file)
        : taskId(id), fileName(file), startTime(std::chrono::system_clock::now()),priority(0),status(RotationTaskStatus::Pending) {}
 ...
 * @details 包含异步轮转相关的配置选项
 */
struct AsyncRotationConfig {
    size_t                      maxQueueSize;                        /*!< 最大队列大小 */
    size_t                      workerThreadCount;                   /*!< 工作线程数量 */
    std::chrono::milliseconds   taskTimeout;                         /*!< 任务超时时间 */
    std::chrono::milliseconds   queueTimeout;                        /*!< 队列等待超时 */
    bool                        enablePreCheck;                      /*!< 启用预检查 */
    bool                        enableTransaction;                   /*!< 启用事务机制 */
    bool                        enableStateMachine;                  /*!< 启用状态机 */
    size_t                      maxRetryCount;                       /*!< 最大重试次数 */
    std::chrono::milliseconds   retryDelay;                          /*!< 重试延迟 */

    AsyncRotationConfig():
        maxQueueSize(1000), workerThreadCount(2), taskTimeout(60000), queueTimeout(5000),
        enablePreCheck(true), enableTransaction(true), enableStateMachine(true), maxRetryCount(3), retryDelay(1000) {}
};
...
public:
    ...
    RotationTrigger CheckRotationNeeded(const std::wstring& currentFileName, size_t fileSize) const override;

    RotationResult PerformRotation(const std::wstring& currentFileName, const RotationTrigger& trigger) override;

    RotationResult ForceRotation(const std::wstring& currentFileName, const std::wstring& reason = L"Manual rotation") override;
    ...
    std::future<RotationResult> PerformRotationAsync(const std::wstring& currentFileName, const RotationTrigger& trigger) override;
     ...
     * @return 异步结果
     */
    std::future<RotationResult> ForceRotationAsync(const std::wstring& currentFileName,const std::wstring& reason = L"Manual rotation");
...
private:
    ...
    // Task Queue-------------------------------------------------------------------
        ...
        std::priority_queue<AsyncRotationRequest,
            std::vector<AsyncRotationRequest>,
    ...
    // Task Management--------------------------------------------------------------
        ...
        std::map<std::wstring,
            std::shared_ptr<RotationTaskInfo>>                  activeTasks_;
