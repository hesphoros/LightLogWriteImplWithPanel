...
// LogLevel, LogCallbackInfo, LogCallback, and CallbackHandle are now defined in LogCommon.h
...
public:
	/**
	 * @brief Set the log filter for filtering log messages
	 * @param filter The filter to apply (can be nullptr to disable filtering)
	 * @details This function sets a filter that will be applied to all log messages
	 *          before they are queued for writing. Messages that don't pass the filter
	 *          will be discarded.
	 */
	 ...
	 * @return Pointer to the current filter, or nullptr if no filter is set
	 */
	std::shared_ptr<ILogFilter> GetLogFilter() const;

	/**
	 * @brief Clear the current log filter (disable filtering)
	 * @details This is equivalent to calling SetLogFilter(nullptr)
	 */
	void ClearLogFilter();

	/**
	 * @brief Check if a log filter is currently active
